...
OxideJS 采用多维度 benchmark 体系，针对 Agent 场景的特性进行验证：
...
- **当前** (2026-06-28): 9,462 pass / 51,834 ran = **18.3%**
...
| 引擎 | test262 pass% | JetStream 2.0 | 启动 (ms) | 内存 (MB) | 备注 |
|------|--------------|---------------|-----------|-----------|------|
| **QuickJS** | ~99% | 基准 (1.0x) | 冷 80, 热 40 | ~120 | ES2020 几乎全覆盖 |
| **Boa v0.21** | ~85% | 0.6x | 冷 150, 热 90 | ~200 | Rust 实现，栈式 VM |
| **JerryScript** | ~98% | 0.8x | 冷 60, 热 30 | ~80 | 嵌入式优化 |
| **OxideJS (当前)** | 18.3% | 未测 | 未测 | 未测 | Phase 14.2.3 |
| **OxideJS (目标)** | 60%+ | 0.8–1.2x | 冷 < 100, 热 < 50 | < 150 | v1.0 里程碑 |

### 4.5 当前状态分析

**test262 通过率 18.3%** 的主要失败原因（基于 `.planning/codebase/CONCERNS.md` 分析）：

1. **Compiler counter/emitter drift** (P0, 预计影响 4000–6000 测试)
   - 问题：`counter/*` 和 `emitter/*` 指令计数不对齐 → `projected_pc` 漂移 → 跳转落错 PC → 死循环
   - 已知 9 个具体 drift 位置：私有方法 (C2/C3)、计算键 (C4/C5)、解构参数 (C1/D1/S2)、`??` 和复合赋值 (E1/E2)

2. **Structural hash cache collision** (P1, 预计影响 2000–4000 测试)
   - 问题：`code_forge.rs` 用 `structural_hash(program)` 做 bytecode cache key，但 hash 逻辑忽略 identifier 名字 → 不同变量引用拿到错误的缓存字节码

3. **缺失高频 builtin** (预计影响 1500–2500 测试)
   - `Array.prototype.toSorted/toReversed/toSpliced/with` (ES2023 不可变数组)
   - `Object.groupBy/Map.groupBy` (ES2024)
   - 其他零散 builtin (`String.isWellFormed`, `Atomics.pause`)

4. **for-in/for-of 枚举顺序** (预计影响 1000–1800 测试)
   - ES2015+ 规定 own-property 枚举顺序（integer keys 升序 → 字符串 keys 插入序 → symbols）

### 4.6 测试复现命令

```bash
# 正确性测试 (全量，约 1 小时)
cargo run --release -p oxide_test262 -- --supervise "tests/test262/test" ""

# 代表性桶测试 (约 5 分钟)
cargo run --release -p oxide_test262 -- "tests/test262/test" "language/expressions/class"

# 性能基准测试
python3 benchmark/run_benchmark.py

# 启动速度测试 (20 次取平均)
for i in {1..20}; do time cargo run --release -- eval "1;"; done

# 内存占用测试
cargo run --release -- run tests/stress/array.js &
PID=$!; while kill -0 $PID 2>/dev/null; do ps -p $PID -o rss=; sleep 0.1; done
```
