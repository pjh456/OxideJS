...
fn rfind_units(hay: &[u16], needle: &[u16], end: usize) -> Option<usize> {
    let hay = &hay[..end];
    if needle.len() == 1 {
...
fn code_point_count(units: &[u16]) -> usize {
    let mut n = 0;
    let mut i = 0;
    while i < units.len() {
...
fn take_code_points(units: &[u16], count: usize) -> &[u16] {
    let mut n = 0;
    let mut i = 0;
...
fn unit_to_byte(s: &str, unit: usize) -> usize {
    if s.is_ascii() {
...
fn map_well_formed_segments(units: &[u16], mut f: impl FnMut(&str) -> String) -> Vec<u16> {
...
/// 时调用方先 `into_owned` 落地。
fn this_units<'a, H: VmHost>(vm: &'a mut H, args: &[u8]) -> Result<Cow<'a, [u16]>, JsValue> {
...
/// null/undefined/symbol 抛 TypeError；对象 ToString 抛出的原生异常原样传播。
fn this_text<'a, H: VmHost>(vm: &'a mut H, args: &[u8]) -> Result<MatchText<'a>, JsValue> {
    let this_val = vm.reg(args[0]);
    if this_val.is_null() || this_val.is_undefined() {
        return Err(crate::error::create_type_error(vm, "String.prototype method called on null or undefined"));
    }
...
fn as_units<'a, H: VmHost>(vm: &'a mut H, val: JsValue) -> Result<Cow<'a, [u16]>, JsValue> {
    if val.is_string() {
