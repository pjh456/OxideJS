...
/// 6. 分配 OBJ_TYPE_DATA_VIEW 对象并挂 DataViewData 状态盒。
///
/// # 边界与前提
/// - 原型读取在全部校验之后：越界 RangeError 先于抛错的 prototype getter。
...
pub fn data_view_constructor<H: VmHost>(vm: &mut H, args: &[u8]) -> NativeResult {
    ...
    let byte_length = if args.len() > 3 {
        let bl_val = vm.reg(args[3]);
        if bl_val.is_undefined() {
            default_len
        } else {
            native_try!(to_index(vm, bl_val, "DataView byteLength out of bounds"))
        }
    } else {
        default_len
    };
...
///
/// 步序：品牌 → ToIndex(byteOffset) → ToInt32(value) 回卷 → 越界 RangeError → 写。
...
///
/// 步序：品牌 → ToIndex(byteOffset) → ToBigInt(value) → 越界 RangeError → 写。
