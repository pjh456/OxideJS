//! RegAlloc 日志宏：`regalloc_error!/warn!/info!/debug!/trace!`，经由 oxide_log 上报。
//! 编译管线属 Kernel 子系统，`OXIDE_LOG=oxide::kernel=debug` 控制本 crate 日志。

#[macro_export]
macro_rules! regalloc_error {
    ($($arg:tt)*) => {
