...
impl Compiler {
    ...
    pub(crate) fn count_array_assignment(&self, ap: &ArrayAssignmentTarget, ctx: &mut CompileCtx) {
        ctx.count_instr(); // FOR_OF_INIT
        for elem in &ap.elements {
            ctx.alloc_reg();
            ctx.count_instr(); // FOR_OF_DONE
    ...
    pub(crate) fn count_object_assignment(&self, op: &ObjectAssignmentTarget, ctx: &mut CompileCtx) {
        for prop in &op.properties {
            match prop {
    ...
    pub(crate) fn count_assignment_maybe_default(&self, target: &AssignmentTargetMaybeDefault, ctx: &mut CompileCtx) {
        match target {
            ...
            AssignmentTargetMaybeDefault::ArrayAssignmentTarget(ap) => self.count_array_assignment(ap, ctx),
            AssignmentTargetMaybeDefault::ObjectAssignmentTarget(op) => self.count_object_assignment(op, ctx),
    ...
    pub(crate) fn count_assign_target(&self, target: &AssignmentTarget, ctx: &mut CompileCtx) {
        match target {
            AssignmentTarget::AssignmentTargetIdentifier(_) => {
                ctx.alloc_reg();
                ctx.count_instr(); // STORE_VAR
            }
            AssignmentTarget::ArrayAssignmentTarget(ap) => self.count_array_assignment(ap, ctx),
            AssignmentTarget::ObjectAssignmentTarget(op) => self.count_object_assignment(op, ctx),
            _ => {}
        }
    }
