...
impl Compiler {
    pub(crate) fn count_object_property_read_static(&self, ctx: &mut CompileCtx) {
        ctx.count_load_var();
        ctx.count_load_const();
        ctx.count_ic_instr_with_ext();
    }

    pub(crate) fn count_object_property_read_key(&self, key: &PropertyKey, computed: bool, ctx: &mut CompileCtx) {
        if !computed {
    ...
    pub(crate) fn count_optional_guard(&self, ctx: &mut CompileCtx) {
        ctx.count_load_var();
        ctx.count_jump();
    }

    pub(crate) fn count_static_member_get_preserve_base(
    ...
    pub(crate) fn count_computed_member_get_preserve_base(
    ...
    pub(crate) fn count_chainable_expression(&self, expr: &Expression, short_chain: bool, ctx: &mut CompileCtx) {
        match expr {
            Expression::StaticMemberExpression(member) => {
                self.count_static_member_get_preserve_base(member, short_chain, ctx)
            }
    ...
    pub(crate) fn count_chain_element(&self, element: &ChainElement, short_chain: bool, ctx: &mut CompileCtx) {
        match element {
    ...
    pub(crate) fn count_logical_assign_test(&self, op: LogicalOperator, id: u32, ctx: &mut CompileCtx) {
        match op {
