...
fn bind_error_subtype_constructor(
    ...
    let mut ctor = Box::new(JsObject::new_empty(
        EMPTY_SHAPE_ID,
        JsValue::from_js_object(function_proto_ptr),
    ));
    ...
    ctor.ensure_hash_props()
        .push(Box::new(JsValue::from_js_object(proto_ptr)));
    ctor.ensure_hash_props()
        .push(Box::new(JsValue::string(name_si, 0)));
    ...
    proto
        .ensure_hash_props()
        .push(Box::new(JsValue::from_js_object(ctor_ptr)));
