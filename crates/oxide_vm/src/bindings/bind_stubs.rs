...
/// 未实现内置的 stub 表：（全局名、native 函数项指针、形参个数、原型 @@toStringTag）。
///
/// `tag` 为 `None` 的条目（Proxy）原型对象不设 @@toStringTag；四弱族各取族名。
const STUBS: [(&str, *const (), u8, Option<&str>); 5] = [
    ("Proxy", oxide_builtins::stubs::proxy_stub::<crate::vm::Vm> as *const (), 2, None),
    (
        "WeakMap",
...
///
/// # 副作用
/// 每个 stub 按规范补装构造器面属性：`prototype`（新建 plain 对象，原型链落
/// Object.prototype，其上带指回 stub 的 `constructor`，四弱族另带 @@toStringTag）、
/// `length`、`name`；描述符均不可枚举，`prototype` 另不可写、不可配置。
...
pub fn bind_stubs(core: &Arc<KernelCore>, session: &mut KernelSession, global: &mut JsObject) {
    ...
    for (name, native_fn, arg_count, tag) in STUBS {
        ...
        // 原型对象：constructor 回指 + @@toStringTag（四弱族）；属性装完再登记
        // 释放登记表，登记点世代即洁净基线。
