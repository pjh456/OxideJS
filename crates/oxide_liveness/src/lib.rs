//! liveness 分析 pass：IRFunction + CFG → LiveInfo（只读，D-15）。
//!
//! 消费 `oxide_ir::IRFunction`（只读）与 `oxide_cfg::Cfg`（preds/succs + exit 哨兵，D-12），
//! 产出 LiveInfo 分析视图（D-01：pass 输出不住进 IR）。两阶段：`dataflow`（块级
//! liveIn/liveOut 不动点）→ `ranges`（块内反向扫描逐指令 live 集）。gen/kill 直接消费
//! contract.rs def_reg/use_regs（D-22），None→0 / This→254 / NewTarget→255 零重复映射。
...
/// 计算 liveness：`&IRFunction + &Cfg → LiveInfo`。纯函数，只读 IR（D-15）。
/// 空 IRFunction 退化：返回 LiveInfo::new()（照 oxide_dce 空函数先例）。
